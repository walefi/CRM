...
export class TasksService {
  ...
  async updateTask(id: string, tenantId: string, dto: UpdateTaskDto, userId?: string) {
    ...
    if (dto.status === 'DONE') {
      this.eventBus.publish(new TaskCompletedEvent(task as any, tenantId, userId))
        .catch((error: any) => this.logger.warn(`Failed to publish TaskCompletedEvent: ${error.message}`));
    }
  ...
  async getProjects(tenantId: string, dto: any) {
    ...
    const [data, total] = await Promise.all([
      prismaAny.project.findMany({ where, skip, take: limit, orderBy: { updatedAt: 'desc' }, include: { tasks: { select: { id: true, title: true, status: true }, take: 5 }, milestones: true, _count: { select: { tasks: true } } } }),
  ...
  async getProject(tenantId: string, id: string) {
    return (this.prisma as any).project.findFirst({
      where: { id, tenantId, deletedAt: null },
      include: { tasks: { orderBy: { order: 'asc' }, include: { assignee: { select: { id: true, firstName: true, lastName: true } } } }, milestones: { orderBy: { order: 'asc' } } },
    });
  }

  async createProject(tenantId: string, userId: string, dto: any) {
    return (this.prisma as any).project.create({
      data: { name: dto.name, description: dto.description, type: dto.type || 'custom', color: dto.color, startDate: dto.startDate ? new Date(dto.startDate) : undefined, endDate: dto.endDate ? new Date(dto.endDate) : undefined, tenantId, createdBy: userId },
    });
  }

  async updateProject(tenantId: string, id: string, dto: any) {
    const prismaAny = this.prisma as any;
    const existing = await prismaAny.project.findFirst({ where: { id, tenantId, deletedAt: null } });
  ...
  async getMilestones(tenantId: string, projectId: string) {
    return (this.prisma as any).milestone.findMany({ where: { projectId }, orderBy: { order: 'asc' } });
  }

  async createMilestone(tenantId: string, dto: any) {
    return (this.prisma as any).milestone.create({ data: { projectId: dto.projectId, title: dto.title, dueDate: dto.dueDate ? new Date(dto.dueDate) : undefined, order: dto.order || 0 } });
  }

  // Dependencies
  async createDependency(tenantId: string, dto: any) {
    return (this.prisma as any).taskDependency.create({ data: { taskId: dto.taskId, dependsOnId: dto.dependsOnId, type: dto.type || 'finish_to_start' } });
  }

  async getDependencies(tenantId: string, taskId: string) {
    return (this.prisma as any).taskDependency.findMany({ where: { taskId }, include: { dependsOn: { select: { id: true, title: true, status: true } } } });
  }
  ...
  async getWorkload(tenantId: string, userId?: string) {
    ...
    const tasks = await this.prisma.task.findMany({ where, include: { assignee: { select: { id: true, firstName: true, lastName: true } }, project: { select: { id: true, name: true, color: true } } } });
    ...
    tasks.forEach(t => {
      ...
      entry.taskCount++;
      entry.tasks.push({ id: t.id, title: t.title, status: t.status, priority: t.priority, dueDate: t.dueDate, project: (t as any).project });
    });
