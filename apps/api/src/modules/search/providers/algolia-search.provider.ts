...
@Injectable()
export class AlgoliaSearchProvider implements ISearchProvider {
  ...
  async search(_tenantId: string, query: SearchQuery): Promise<SearchResponse> {
    this.logger.warn('Algolia provider not configured');
    return {
      data: [],
      meta: {
        total: 0,
  ...
  async healthCheck(): Promise<{ status: string; provider: string }> {
    return { status: 'not_configured', provider: 'algolia' };
  }
}
