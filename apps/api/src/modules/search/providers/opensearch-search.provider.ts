...
@Injectable()
export class OpensearchSearchProvider implements ISearchProvider {
  ...
  async search(_tenantId: string, query: SearchQuery): Promise<SearchResponse> {
    this.logger.warn('OpenSearch provider not configured');
    return {
      data: [],
      meta: {
        total: 0,
  ...
  async healthCheck(): Promise<{ status: string; provider: string }> {
    return { status: 'not_configured', provider: 'opensearch' };
  }
}
